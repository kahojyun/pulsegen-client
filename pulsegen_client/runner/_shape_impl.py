...
def get_shape(info: _shape.ShapeInfo) -> PulseShape:
    if isinstance(info, _shape.HannShape):
        return HannShape()
    if isinstance(info, _shape.TriangleShape):
        return TriangleShape()
    if isinstance(info, _shape.InterpolatedShape):
